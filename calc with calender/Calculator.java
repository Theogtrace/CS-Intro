        ...
        while (!done)
        {
           char nextOp = (keyboard.next( )).charAt(0);
            ...
            else if ((nextOp == 'm') || (nextOp == 'M'))
            {
             	try
        		{
        			outputDevice = new PrintWriter ("Data.txt");
            ...
            else if ((nextOp == 'r') || (nextOp == 'R'))	
            {
            	try
    			{
    				inputDevice = new Scanner (new File ("Data.txt"));
    				double num = inputDevice.nextDouble();
    				inputDevice.close();
    				System.out.println("Recal memory value" + num);
    			}
    			catch (FileNotFoundException ex)
    			{
    				System.out.println ("fail to open file");
